...
# Explicitly exclude individual sensors here so they don't enter the global models
excluded_vars <- c("PeopleId", "chest.temperature...C.", "hand.temperature...C.", "ankle.temperature...C.")

train <- df[idx, ] %>% select(-all_of(excluded_vars))
test  <- df[-idx, ] %>% sample_n(n) %>% select(-all_of(excluded_vars))
```
